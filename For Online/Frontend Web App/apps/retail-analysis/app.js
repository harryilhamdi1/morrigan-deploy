...
function loadStoreDetail(idOverride) {
    ...
    if (isUnassessed) {
        ...
        Plotly.newPlot("stRadarChart", [
            {
                type: 'scatterpolar', r: rSt, theta: radarKeys.map(k => k ? k.split(' ')[0] : ''),
                fill: 'toself', fillcolor: 'rgba(30, 58, 138, 0.2)', // Darker blue fill
                name: 'Store', line: { color: '#1E3A8A', width: 3 }, marker: { size: 1 }
            },
            {
                type: 'scatterpolar', r: rComp, theta: radarKeys.map(k => k ? k.split(' ')[0] : ''),
...
window.onload = function () {
    // Note: Do not fire legacy initializations here. 
    // They are now handled exclusively by the Supabase adapter at the bottom of the file (DOMContentLoaded).
    // Firing them here causes race conditions and TypeError because data might not be loaded yet or might be empty.
    ...
    if (urlParams.get('mode') === 'standalone') {
        ...
        if (targetStoreName) {
            try {
                ...
                if (targetId) {
                    showTab('stores');
                    // Delay viewStore to let adapter finish if it's still running
                    setTimeout(() => viewStore(targetId), 500);
                } else {
        ...
        // Default tab handled by adapter
    }
};
...
// ==========================================
// SUPABASE LIVE DATA ADAPTER (Phase 4)
// ==========================================
...
window.reportData = {
    ...
    stores: {},
    threshold: 86,
    actionPlanConfig: {}, // Can leave empty, fallback messages used
    voc: [],
    failureReasons: []
};
...
document.addEventListener("DOMContentLoaded", async () => {
    // 1. Session Check & Initial Loader
    ...
    // Replace the main body content temporarily with a loader
    ...
    mainContent.innerHTML = `
        <div class="d-flex flex-column justify-content-center align-items-center h-100" style="min-height: 80vh;">
            ...
            <h4 class="mt-4 fw-bold" style="font-family: 'Outfit', sans-serif;">Synchronizing Matrix</h4>
            <p class="text-muted">Fetching live data from Supabase...</p>
        </div>
    `;

    try {
        // Fetch Profile for RBAC (for later, though Retail Analysis is mostly view-only)
        const { data: profile } = await supabaseClient.from('profiles').select('*').eq('id', session.user.id).maybeSingle();
        window.userProfile = profile;

        // 2. Fetch Data
        // Stores
        const { data: stores, error: sErr } = await supabaseClient.from('stores').select('*');
        if (sErr) throw sErr;

        // Helper function to fetch > 1000 rows
        ...
        // KPI Scores (Base Table)
        const { data: kpiScoresRaw, error: kErr } = await supabaseClient.from('kpi_scores').select('*');
        if (kErr) throw kErr;

        // Fetch all related Journey Scores using Pagination (Bypass 1000 limit)
        const allJourneyScores = await fetchAllRows('journey_scores');

        // Fetch all related Granular Scores using Pagination (Bypass 1000 limit)
        const allGranularScores = await fetchAllRows('granular_scores');

        // Manually Join Data
        const kpiScores = kpiScoresRaw.map(kpi => {
            return {
                ...kpi,
                journey_scores: allJourneyScores.filter(js => js.kpi_score_id === kpi.id),
                granular_scores: allGranularScores.filter(gs => gs.kpi_score_id === kpi.id)
            };
        });

        // Action Plans (To use for National Priorities / VoC fallback)
        const { data: actionPlans, error: aErr } = await supabaseClient.from('action_plans').select('*, stores(*)');
        if (aErr) throw aErr;

        // 3. Reconstruct Stores Dictionary
        stores.forEach(st => {
            window.reportData.stores[st.site_code] = {
                meta: {
                    code: st.site_code,
                    name: st.store_name,
                    region: st.region,
        ...
        // 4. Group KPI Scores into Waves and Stores
        let allWavesSet = new Set();

        kpiScores.forEach(kpi => {
            const wKey = `${kpi.wave_year} ${kpi.wave_name}`;
            allWavesSet.add(wKey);

            // Reconstruct sections from journey_scores relation
            let parsedSections = {};
            ...
            // Reconstruct failedItems from granular_scores
            let parsedFailedItems = [];
            if (kpi.granular_scores && Array.isArray(kpi.granular_scores)) {
                kpi.granular_scores.forEach(gs => {
                    if (gs.score === 0 || gs.score === false || gs.score === "0") {
                        parsedFailedItems.push({
                            code: gs.item_code,
                            item: gs.item_name,
            ...
            // Reconstruct details for deeper drilldowns
            let parsedDetails = {};
            if (kpi.granular_scores && Array.isArray(kpi.granular_scores)) {
                kpi.granular_scores.forEach(gs => {
            ...
            // Find matching store
            const st = stores.find(s => s.id === kpi.store_id);
            if (st && window.reportData.stores[st.site_code]) {
                window.reportData.stores[st.site_code].results[wKey] = {
        ...
        window.sortedWaves = Array.from(allWavesSet).sort();

        // 5. Build Aggregations (Summary, Regions, Branches)
        const buildAgg = (levelObj, record, wave, regionOrBranch) => {
            if (!levelObj[wave]) levelObj[wave] = { sum: 0, count: 0, sections: {}, details: {} };
            levelObj[wave].sum += record.totalScore;
            levelObj[wave].count++;

            // Aggregate Sections (A, B, C...)
            Object.entries(record.sections).forEach(([secName, val]) => {
                if (!levelObj[wave].sections[secName]) levelObj[wave].sections[secName] = { sum: 0, count: 0, critical: 0 };
                levelObj[wave].sections[secName].sum += val;
                levelObj[wave].sections[secName].count++;
                if (val < window.reportData.threshold) levelObj[wave].sections[secName].critical++;
            });

            // Aggregate Details (Granular Items)
            if (record.details) {
                if (!levelObj[wave].details) levelObj[wave].details = {};
                Object.entries(record.details).forEach(([secLetter, items]) => {
                    if (!levelObj[wave].details[secLetter]) levelObj[wave].details[secLetter] = {};

                    Object.entries(items).forEach(([itemCode, itemObj]) => {
                        if (!levelObj[wave].details[secLetter][itemCode]) {
                            levelObj[wave].details[secLetter][itemCode] = { t: itemObj.t, sum: 0, count: 0 };
                        }

                        // Parse score (could be boolean, string, or int)
                        let val = 0;
                        if (itemObj.r === 1 || itemObj.r === "1" || itemObj.r === true) val = 1;
                        if (itemObj.r === 0 || itemObj.r === "0" || itemObj.r === false) val = 0;

                        // If it's a valid 1 or 0, count it
                        if (val === 1 || val === 0) {
                            levelObj[wave].details[secLetter][itemCode].sum += val;
                            levelObj[wave].details[secLetter][itemCode].count++;
                        }
                    });
                });
            }
        ...
        Object.values(window.reportData.stores).forEach(st => {
            Object.entries(st.results).forEach(([wKey, res]) => {
                buildAgg(window.reportData.summary, res, wKey);

                if (!window.reportData.regions[st.meta.region]) window.reportData.regions[st.meta.region] = {};
                buildAgg(window.reportData.regions[st.meta.region], res, wKey);

                if (!window.reportData.branches[st.meta.branch]) window.reportData.branches[st.meta.branch] = {};
                buildAgg(window.reportData.branches[st.meta.branch], res, wKey);
            });
        });

        // 6. Map Action Plans back into the UI for VoC & Priorities
        // The original script relies on reportData.voc, but we can fake reportData.voc using Action Plans that have "Suara Pelanggan" type!
        actionPlans.forEach(ap => {
            if (ap.category.includes('Suara Pelanggan') || ap.category.includes('VOC')) {
                window.reportData.voc.push({
                    siteCode: ap.stores.site_code,
                    siteName: ap.stores.store_name,
                    region: ap.stores.region,
                    branch: ap.stores.branch,
                    text: ap.action_required,
                    themes: [ap.finding_source.split(':')[0]], // Make a fake theme classification
                    aiInsight: 'N/A',
                    sentiment: 'negative',
                    wave: window.sortedWaves[window.sortedWaves.length - 1] // Assign to latest wave
                });
            }
        });

        // National Priorities relies on `reportData.summary[curWave].details`
        // In the adapter, we will patch `initSummary` and `initRegions` below to use actionPlans directly if details are missing.

        // Restore HTML
        mainContent.innerHTML = originalTabs;

        // Setup Top Navbar Name (Legacy sidebar removal adapter)
        ...
        if (window.sortedWaves.length === 0) {
            mainContent.innerHTML = `<div class="d-flex flex-column align-items-center justify-content-center h-100 p-5 mt-5">
                <i class="bi bi-inbox text-muted" style="font-size: 4rem;"></i>
                <h4 class="mt-3 fw-bold text-dark">No Data Available</h4>
                <p class="text-muted text-center" style="max-width: 500px;">
                    There are no KPI scores available in the database yet. Please upload the data or run the initial database migration.
                </p>
            </div>`;
            return;
        }
        ...
        initRegions();
        initBranches();
        if (typeof initStoreTable === 'function') initStoreTable();
        if (typeof initVoc === 'function') initVoc();

        // Remove active class from other tabs except summary
        const allLinks = document.querySelectorAll('.sidebar-nav a');
        allLinks.forEach(l => l.classList.remove('active'));
        const activeLink = document.querySelector('.sidebar-nav a[onclick*="summary"]');
        if (activeLink) activeLink.classList.add('active');

    } catch (err) {
        console.error("Critical Adapter Error:", err);
        mainContent.innerHTML = `
            <div class="alert alert-danger m-5">
                <h4>Connection Error</h4>
                <p>Failed to sync matrix with Supabase: \${err.message}</p>
