...
var storeListState = {
    page: 1,
    perPage: 15, // Comfortable pagination size
    search: "",
    region: "",
    branch: "",
    sort: "score_desc"
...
function updateStoreState(key, val) {
    ...
    storeListState.page = 1; // Reset to page 1 on filter
    renderStoreTable();
}
...
function resetStoreFilters() {
    ...
    document.getElementById("storeDetailPanel").style.display = 'none'; // Hide detail
    renderStoreTable();
}
...
function renderStoreTable() {
    ...
    pageData.forEach(function (s, idx) {
        ...
        var badgeStyle = score >= 90 ? "background-color:#002060 !important" : ""; // Override for custom primary
        ...
        tr.innerHTML = `
            ...
            <td class="text-end pe-4">
                <button class="btn btn-sm btn-outline-secondary" onclick="event.stopPropagation(); loadModernStoreDetail('${s.meta.code}')">View Details</button>
            </td>
        `;
...
function loadModernStoreDetail(siteCode) {
    ...
    // Build Detail View using existing logic but modernized
    container.innerHTML = `
        <div class="card border-0 shadow-lg overflow-hidden">
            ...
            <div class="card-body p-4">
                <div class="row g-4">
                    <div class="col-lg-8">
                        ...
                        <div id="stTrendChartModern" style="height:350px"></div>
                    </div>
                     <div class="col-lg-4">
                        <h5 class="fw-bold text-primary-custom mb-3">Critical Issues (< 84)</h5>
                        <div id="stIssuesList" class="vstack gap-3" style="max-height:350px;overflow-y:auto"></div>
                    </div>
                </div>
                
                <h5 class="fw-bold text-primary-custom mt-4 mb-3">Detailed Section Breakdown</h5>
                <div class="table-responsive">
                    <table class="table table-sm table-hover" id="stSectionTableModern">
            ...
            <div class="card-footer bg-light p-3 text-end">
                <button class="btn btn-outline-secondary btn-sm" onclick="document.getElementById('storeDetailPanel').style.display='none'">Close Detail</button>
            </div>
        </div>
    `;
    ...
    Plotly.newPlot("stTrendChartModern", [{
    ...
    // Render Issues & Table
    var tableBody = document.querySelector("#stSectionTableModern tbody");
    var issuesDiv = document.getElementById("stIssuesList");
