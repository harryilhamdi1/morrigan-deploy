...
function generateHTML(data) {
    ...
    var parts = [
        ...
        '    </div>',
        '    <!-- Row 3: Leaderboard & Deep Dive Tiles -->',
        '    <h4 class="text-primary-custom mb-3 mt-5">🏆 Regional Leaderboard & Focus Areas</h4>',
        ...
        '       legDiv.id = "hmLegend";',
        '       legDiv.className = "d-flex justify-content-center gap-3 mb-3 small";',
        '       legDiv.innerHTML = `',
        '           <div class="d-flex align-items-center"><span style="width:12px;height:12px;background:#EF4444;border-radius:2px;margin-right:6px;"></span>Critical (<84)</div>',
        '           <div class="d-flex align-items-center"><span style="width:12px;height:12px;background:#FCD34D;border-radius:2px;margin-right:6px;"></span>Warning (84-90)</div>',
        '           <div class="d-flex align-items-center"><span style="width:12px;height:12px;background:#60A5FA;border-radius:2px;margin-right:6px;"></span>Good (90-95)</div>',
        '           <div class="d-flex align-items-center"><span style="width:12px;height:12px;background:#002060;border-radius:2px;margin-right:6px;"></span>Excellent (>95)</div>`;',
        ...
        '   }',
        '',
        '   Plotly.newPlot("regionSectionHeatmap", [{',
        '       x: secKeys.map(s => s.split(" ")[0]), y: regKeys, z: z,',
        '       type: "heatmap",',
        '       colorscale: [[0, "#EF4444"], [0.5, "#FCD34D"], [0.8, "#60A5FA"], [1, "#002060"]],',
        '       xgap: 2, ygap: 2,',
        '       hovertemplate: "<b>Region: %{y}</b><br>Section: %{x}<br>Score: %{z:.2f}<extra></extra>"',
        '   }], { margin: {l: 120, b: 50, t: 20}, xaxis: {side: "bottom"} }, config);',
        '',
        '   // Interactivity: Click to Deep Dive',
        '   document.getElementById("regionSectionHeatmap").on("plotly_click", function(data){',
        ...
        '           m.innerHTML = `',
        '           <div class="modal fade" id="heatmapModal" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content border-0 shadow-lg" style="border-radius:16px;overflow:hidden;">',
        '               <div class="modal-header bg-primary text-white border-0" style="background:linear-gradient(135deg,#002060 0%,#1e3a8a 100%);"><div><h5 class="modal-title fw-bold" id="hmModalTitle">Analysis</h5><small class="opacity-75" id="hmModalSubtitle">Detail</small></div><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>',
        '               <div class="modal-body p-4 bg-light"><div class="row g-4">',
        '                   <div class="col-md-5"><div class="card border-0 shadow-sm h-100"><div class="card-body text-center p-4">',
        '                       <div class="text-uppercase text-muted small fw-bold mb-2">Current Score</div><div class="display-3 fw-bold mb-2" id="hmScore">--</div><div id="hmBadge"></div>',
        '                       <div class="mt-4 pt-3 border-top"><div class="small text-muted mb-1">vs National Avg</div><div class="h5 fw-bold" id="hmVsNat">--</div></div>',
        '                   </div></div></div>',
        '                   <div class="col-md-7"><div class="card border-0 shadow-sm h-100"><div class="card-header-clean py-3">📉 The Culprits (Bottom 5 Stores)</div><div class="card-body p-0"><table class="table table-sm table-hover mb-0" style="font-size:0.85rem;"><thead class="table-light"><tr><th>Store</th><th class="text-end">Score</th></tr></thead><tbody id="hmCulprits"></tbody></table></div></div></div>',
        '                   <div class="col-12"><div class="card border-0 shadow-sm"><div class="card-body d-flex align-items-center"><div class="me-3 display-6">💡</div><div><h6 class="fw-bold mb-1">Recommended Action</h6><p class="mb-0 text-muted small" id="hmAction">--</p></div></div></div></div>',
        '                   <div class="col-12"><div class="card border-0 shadow-sm"><div class="card-header-clean py-3 small">📊 Historical Trend</div><div class="card-body p-2"><div id="hmTrendChart" style="height:150px;"></div></div></div></div>',
        '               </div></div>',
        '               <div class="modal-footer border-0 bg-white"><button type="button" class="btn btn-primary px-4 rounded-pill" data-bs-dismiss="modal">Close</button></div>',
        '           </div></div></div>`;',
        '           document.body.appendChild(m.firstChild);',
        '           // Need to load Bootstrap JS? It is already loaded via CDN.',
        '       }',
        '',
        '       // Populate Modal',
        '       document.getElementById("hmModalTitle").textContent = r + " - " + sDisplay;',
        ...
        '       scEl.className = "display-3 fw-bold mb-2 " + (score<84?"text-danger":"text-primary");',
        '       document.getElementById("hmBadge").innerHTML = score<84?`<span class="badge bg-danger">CRITICAL</span>`:(score<90?`<span class="badge bg-warning text-dark">WARNING</span>`:`<span class="badge bg-success">GOOD</span>`);',
        '       document.getElementById("hmVsNat").innerHTML = (diff>=0?`<span class="text-success">▲ +`:`<span class="text-danger">▼ `) + diff.toFixed(2) + "</span>";',
        '       document.getElementById("hmAction").textContent = reportData.actionPlanConfig[fullSec] || "Review operational standards.";',
        '',
        ...
        '       Plotly.newPlot("hmTrendChart", [{x:waves,y:yTrend,type:"scatter",mode:"lines+markers",line:{color:"#002060",width:2},marker:{color:"#002060",size:6}}], {margin:{t:10,l:30,r:10,b:20},height:150,xaxis:{showgrid:false},yaxis:{showgrid:true,gridcolor:"#eee"}}, config);',
        '',
